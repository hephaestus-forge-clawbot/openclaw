...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("hephie models auth setup-token")}\` or \`${formatCliCommand("hephie configure")}\`.`
          : `Run \`${formatCliCommand("hephie configure")}\` or set an API key env var.`;
