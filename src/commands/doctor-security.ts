...
export async function noteSecurityWarnings(cfg: OpenClawConfig) {
  ...
  const auditHint = `- Run: ${formatCliCommand("hephie security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("hephie configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("hephie config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("hephie doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("hephie config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
