...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("hephie node install"), formatCliCommand("hephie node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("hephie node install --force")}`);
    }
    return;
  }
