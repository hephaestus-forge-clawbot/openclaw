...
const DEFAULT_PACKAGE_NAME = "hephie";
...
function normalizeTag(tag?: string) {
  ...
  if (trimmed.startsWith("hephie@")) {
    return trimmed.slice("hephie@".length);
  }
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "hephie-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "hephie.mjs");
    ...
    const doctorStep = await runStep(
      step("hephie doctor", doctorArgv, gitRoot, { HEPHIE_UPDATE_IN_PROGRESS: "1" }),
    );
